...
SELECT Left((`hosts`), 2) AS 主机名, COUNT(1) AS 账号数, COUNT(login) AS 离线数, MAX(last_run_time) AS 最晚运行时间, MIN(last_run_date) AS 最早运行日期, MAX(last_run_date) AS 最晚运行日期 FROM `idle_fish` GROUP BY Left((`hosts`), 2) ORDER BY Left((`hosts`), 2)
