...
class LDBase:  # pylint: disable=too-few-public-methods
    ...
    def timeout_monitoring(self, start_datetime, timeout=18):
        """超时监控

        :param start_datetime: 开始时间
        :param timeout: 超时退出时间，默认18秒
        """
    ...
    def exe_cmd(self, command='', ext='', timeout=15, return_flag=True):
        """执行命令函数

        :param command: adb命令
        :param ext: 命令的扩展参数
        :param timeout: 超时中断时间，默认15秒
